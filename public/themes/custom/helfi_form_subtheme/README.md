...
Or if you don't accept the theme name, you can rename every `helfi_form_subtheme` file with `your_theme_name` and renaming every `helfi_form_subtheme_*` variable/reference with `your_theme_name_*`. And then enable it in `/admin/appearance`.
...
The icons can be used in twig like so:
    ...
    {% include "@helfi_form_subtheme/misc/icon.twig" with {icon: 'my-awesome-icon'} %}
