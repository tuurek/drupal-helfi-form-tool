...
class FormToolShareController extends ControllerBase {
   ...
   *
   * @see \Drupal\form_tool_share\Theme\FormToolShareThemeNegotiator
   * @see page--form-tool-share.html.twig
   * @see form_tool_share.libraries.yml
   */
  public function page(Request $request, $library = NULL, $version = NULL) {
    ...
    $build['webform'] = [
      ...
      '#prefix' => '<div class="form-tool-share-submission-form">',
      '#suffix' => '</div>',
    ];
    // Attachments.
    $build['#attached']['library'][] = 'form_tool_share/form_tool_share.page';
    if ($library && $version) {
      $build['#attached']['library'][] = "form_tool_share/libraries.$library.$version";
    }
    // Add setting notifying AjaxCommand that this page is shared via an
    // embeddded iframe.
    // @see Drupal.AjaxCommands.prototype.webformRefresh
    $build['#attached']['drupalSettings']['form_tool_share']['page'] = TRUE;
    return $build;
  }
  ...
  public function preview(Request $request) {
    ...
    $build['preview'] = [
      '#type' => 'container',
      '#attributes' => ['class' => ['form-tool-share-iframe-container']],
    ];
    ...
    $build['#attached']['library'][] = 'form_tool_share/form_tool_share.admin';
    return $build;
  }
