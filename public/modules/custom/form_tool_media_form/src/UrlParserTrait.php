...
namespace Drupal\form_tool_media_form;
...
trait UrlParserTrait {
  ...
  protected function getEmbedUrl(string $uri) : string {
    ...
    if ($uri->getHost() === HelForm::FORM_URL) {
    ...
    if ($uri->getHost() === HelForm::PALVELUFORM_URL) {
      // Link is already a valid embed link.
      if (strpos($uri->getPath(), '/embed') !== FALSE) {
        return (string) $uri;
      }
  ...
  /**
   * Parses 'Open form in new window' link from embed URL.
   *
   * @param string $uri
   ...
   *   The url.
   */
  protected function getFormUrl(string $uri) : ? string {
    $uri = Http::createFromString($uri);

    $this->assertMediaLink($uri);

    if ($uri->getHost() === HelForm::FORM_URL) {
      // Link is already a direct form link.
      if (strpos($uri->getPath(), 'embed') === FALSE) {
        return (string) $uri;
      }
